...
const serviceAccount = {
  ...
  }; // Replace with your admin SDK file path
const app = admin.apps.length
  ? admin.app()
  : admin.initializeApp({
      credential: admin.credential.cert(serviceAccount),
      // Add any other necessary Firebase configurations
    });
...
export async function POST(req) {
    ...
    try {
      ...
      return NextResponse.json({ status:200, success: true, message: 'Custom claim set successfully' });
    } catch (error) {
      return NextResponse.json({ success: false, error: 'Failed to set custom claim' });
    }
}
